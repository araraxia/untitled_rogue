...
class popup_menu(Untgap):
    def __init__(
        ...
        self.borderwidth = (
            borderWidth if borderWidth else self.popup_defaults["borderwidth"]
        )
        self.relief = relief if relief else self.popup_defaults["relief"]

        self.max_visible = (
            max_visible if max_visible else self.popup_defaults["max_visible"]
        )
        ...
        self.selected_font = (
            select_font if select_font else self.popup_defaults["selected"]
        )
        self.deselected_font = (
            deselect_font if deselect_font else self.popup_defaults["deselected"]
        )
        self.disabled_font = (
            disabled_font if disabled_font else self.popup_defaults["disabled"]
        )
        ...
        for index, option in enumerate(menu_options):
            ...
            self.menu.append({"text": option[0], "command": option[1], "state": state})

        if not self.menu:
            self.menu.append(
                {"text": "Close", "command": self.kill_popup, "state": "selected"}
            )

        self.logger.info(
            f"Initializing popup menu with {len(self.menu)} items. Jump to: {self.jump_to_text}.")
    ...
    def move_selection(self, direction):
        ...
        for index, line in enumerate(self.menu):
            if line["state"] == "selected":
                ...
                self.logger.debug(f"Moving to: {self.menu[index+direction]['text']}")
    ...
    def _check_pos_size(self):
        ...
        if not self.height:
            menu_length = (
                len(self.menu)
                if len(self.menu) < self.max_visible
                else self.max_visible
            )
            ...
            raw_height = (max_size * menu_length) + (
                self.popup_defaults["leading"] * (menu_length + 1)
            )
    ...
    def init_popup(self):
        self.logger.info(f"Initializing popup menu: {self.current_screen}.")
        ...
        for line in self.menu:
            label = tk.Label(
                self.popup_frame, text=line["text"], font=self.deselected_font
            )
    ...
    def kill_popup(self):
        self.logger.debug("Killing popup menu.")
